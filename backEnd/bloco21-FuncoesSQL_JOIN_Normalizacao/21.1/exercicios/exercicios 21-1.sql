...
/* 4. Escreva uma query que exiba
a quantidade de dinheiro necessária para realizar o pagamento de todas as pessoas funcionárias.*/
...
SELECT JOB_ID, COUNT(JOB_ID) FROM hr.employees WHERE JOB_ID = 'IT_PROG';

/*7. Escreva uma query que exiba
a quantidade de dinheiro necessária para efetuar o pagamento de cada profissão ( JOB_ID ). */
SELECT JOB_ID, SUM(SALARY) FROM hr.employees GROUP BY JOB_ID;

/* 8. Utilizando a query anterior, 
faça as alterações para que seja exibido somente a quantidade de dinheiro necessária
...
WHERE job_id <> 'IT_PROG' GROUP BY job_id ORDER BY `average_salary` DESC;

/* 10. Escreva um query que exiba média salarial
e o número de funcionários de todos os departamentos com mais de dez funcionários.
...
/* 12. Escreva uma query que só exiba
as informações dos funcionários cujo o primeiro nome tenha oito ou mais caracteres. */
...
o sobrenome e a data de contratação de todos os funcionário contratados em julho de 1987. */
