...
SELECT actor.actor_id, actor.first_name, film.film_id FROM sakila.actor AS actor
...
ordenados pelo nome em ordem decrescente, juntamente com o id do endereço e o nome da rua onde o cliente mora.
...
INNER JOIN sakila.address AS A
ON C.address_id = A.address_id
WHERE C.active = 1 GROUP BY C.customer_id ORDER BY C.first_name DESC;
...
WHERE YEAR(P.payment_date) = 2006 GROUP BY S.first_name, S.last_name;
