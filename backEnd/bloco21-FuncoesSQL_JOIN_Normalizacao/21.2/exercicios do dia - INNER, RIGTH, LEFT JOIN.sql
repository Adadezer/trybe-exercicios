...
SELECT M.title, (B.domestic_sales + B.international_sales) AS vendas FROM Pixar.Movies AS M
INNER JOIN
Pixar.BoxOffice AS B
ON id = movie_id
...
SELECT T.name, T.location, M.title, M.director, M.year, M.length_minutes FROM Pixar.Theater AS T
...
Retorne os nomes dos cinemas em ordem alfabética.*/
SELECT M.title, M.director, M.year, M.length_minutes, T.name, T.location FROM Pixar.Theater AS T
RIGHT JOIN Pixar.Movies AS M
ON M.theater_id = T.id
ORDER BY T.name;

 -- Exercício 6: Utilizando o INNER JOIN , selecione todas as informações dos filmes com avaliação maior que 8.
SELECT M.title, M.director, M.year, M.length_minutes FROM Pixar.Movies AS M
